...
void getPLA(int opt, int argc, char **argv, int option, pPLA *PLA, int out_type, int mykey , char newname[]);
...
int main(int argc, char **argv)
{
    ...
    pPLA PLA, PLA1 , PLA2 , PLA3 , PLA4;
    pcover F, Fold , Fold1, Dold , Fold2 , Fold3 , Fold4;
    pset last1, p;
    cost_t cost;
    ...
    while ((i = getopt(argc, argv, "A:B:D:S:de:o:r:stv:x")) != EOF) {
	switch(i) {
		case 'A':
			mykey1 = atoi(optarg);
			break;
    ...
    PLA = PLA1 = PLA2 = PLA3 = PLA4 = NIL(PLA_t);
    switch(option_table[option].num_plas) {
	case 2:
	    if (optind+2 < argc) fatal("trailing arguments on command line");
	    //getPLA(optind++, argc, argv, option, &PLA, out_type,mykey1);
		getPLA(optind, argc, argv, option, &PLA, out_type,mykey1,"a");
	    getPLA(optind, argc, argv, option, &PLA1, out_type,mykey2,"b");
		getPLA(optind, argc, argv, option, &PLA2, out_type,mykey2,"a");
		getPLA(optind, argc, argv, option, &PLA3, out_type,mykey2,"a");
		getPLA(optind++, argc, argv, option, &PLA4, out_type,mykey2,"a");
	    break;
	case 1:
...
/******************** Espresso operations ********************/
	...
	do {
	    ...
	    if (print_solution) {
		fprint_pla(stdout, PLA, out_type,"hi");
		(void) fflush(stdout);
	    }
    ...
    case KEY_qm:
	Fold = sf_save(PLA->F);
	Fold1 = sf_save(PLA1->F);
	Fold2 = sf_save(PLA2->F);
	Fold3 = sf_save(PLA3->F);
	Fold4 = sf_save(PLA4->F);



	PLA->F = minimize_exact(PLA->F, PLA->D, PLA->R, exact_cover);
	PLA1->F = minimize_exact(PLA1->F, PLA1->D, PLA1->R, exact_cover);

	PLA2->F = minimize_exact(PLA2->F, PLA2->D, PLA2->R, exact_cover);
	PLA3->F = minimize_exact(PLA3->F, PLA3->D, PLA3->R, exact_cover);
	PLA4->F = minimize_exact(PLA4->F, PLA4->D, PLA4->R, exact_cover);


	EXECUTE(error=verify(PLA->F,Fold,PLA->D), VERIFY_TIME, PLA->F, cost);
	EXECUTE(error=verify(PLA1->F,Fold1,PLA1->D), VERIFY_TIME, PLA1->F, cost);

	EXECUTE(error=verify(PLA2->F,Fold2,PLA2->D), VERIFY_TIME, PLA2->F, cost);
	EXECUTE(error=verify(PLA3->F,Fold3,PLA3->D), VERIFY_TIME, PLA3->F, cost);
	EXECUTE(error=verify(PLA4->F,Fold4,PLA4->D), VERIFY_TIME, PLA4->F, cost);

	//------------------xor------------------	
	pcover T1 = cv_intersect(PLA->F, PLA1->R);
	pcover T2 = cv_intersect(PLA1->F, PLA->R);
	free_cover(PLA2->F);
	PLA2->F = sf_contain(sf_join(T1, T2));
	free_cover(T1);
	free_cover(T2);
	PLA2->F = minimize_exact(PLA2->F, PLA2->D, PLA2->R, exact_cover);
	EXECUTE(fprint_pla(stdout, PLA2, out_type , "xor.pla"), WRITE_TIME, PLA2->F, cost);
	// //-----------------and-------------------
	pcover T3 = cv_intersect(PLA->F, PLA1->F);
	PLA3->F = T3;
	PLA3->F = minimize_exact(PLA3->F, PLA3->D, PLA3->R, exact_cover);
	EXECUTE(fprint_pla(stdout, PLA3, out_type , "and.pla"), WRITE_TIME, PLA3->F, cost);
	// //-----------------or--------------------
	pcover T4 = sf_contain(sf_join(PLA->F,PLA1->F));
	PLA4->F = T4;
	PLA4->F = minimize_exact(PLA4->F, PLA4->D, PLA4->R, exact_cover);
	EXECUTE(fprint_pla(stdout, PLA4, out_type , "or.pla"), WRITE_TIME, PLA4->F, cost);
...
/******************** Simple cover operations ********************/
    ...
    /* Output the solution */
    if (print_solution) {
	EXECUTE(fprint_pla(stdout, PLA, out_type , "a.pla"), WRITE_TIME, PLA->F, cost);
	printf("------------------------------------------------\n");
	EXECUTE(fprint_pla(stdout, PLA1, out_type, "b.pla"), WRITE_TIME, PLA1->F, cost);
    }
...
void getPLA(int opt, int argc, char **argv, int option, pPLA *PLA, int out_type, int mykey ,char newname[])
{
	...
	char buf_k[255];
	char input_buf_k[255];
	char output_buf_k[255];
	...
	while (fgets(buf_k, sizeof(buf_k), fp) != NULL){
        char search[] = ".i";
        char search1[] = ".o"; 
        char *loc = strstr(buf_k,search);
        char *loc1 = strstr(buf_k,search1);
        if(loc!=NULL){
            strcpy(input_buf_k,buf_k);
            continue;
        }
        if(loc1!=NULL){
            strcpy(output_buf_k,buf_k);
            continue;
        }
        row_count++;
	}
    col_count = strlen(buf_k);
	fclose(fp);

	char data[row_count][col_count];
	FILE *fp1 = fopen(argv[opt], "r");

	int index = 0;
    for(int i=0;i<row_count+2;i++){     //+2是因為.i .o
        if(fgets(buf_k, sizeof(buf_k), fp1) != NULL){
            char search[] = ".i";
            char search1[] = ".o"; 
            char *loc = strstr(buf_k,search);
            char *loc1 = strstr(buf_k,search1);
    ...
    char buf_int[255];
    int index_buf_int=0;
    for(int i=0;i<strlen(input_buf_k);i++){
        if(input_buf_k[i]=='.'||input_buf_k[i]=='i'||input_buf_k[i]==' '||input_buf_k[i]=='\n') continue;
        buf_int[index_buf_int] = input_buf_k[i];
        index_buf_int++;
    }
    input_size = atoi(buf_int);

    //清空buf字串
    memset(buf_int,0,255);
    index_buf_int = 0;

    //得到output size
    for(int i=0;i<strlen(output_buf_k);i++){
        if(output_buf_k[i]=='.'||output_buf_k[i]=='o'||output_buf_k[i]==' '||output_buf_k[i]=='\n') continue;
        buf_int[index_buf_int] = output_buf_k[i];
        index_buf_int++;
    }
    output_size = atoi(buf_int);	
    //取得分成兩個所指定的output function 並且存成陣列
    char PLA_[row_count][col_count];
    int index_PLAi=0;
    int index_PLAj=0;
    ...
    for(int i=0;i<row_count;i++){
        index_PLAj = 0;
        //print 到 input跟後面一個字元完
        for(int j=0;j<input_size+1;j++){
            PLA_[index_PLAi][index_PLAj] = data[i][j];
			//printf("%c",data[i][j]);
            index_PLAj++;
        }
        //print 指定行
        for(int k=input_size+1;k<col_count;k++){
            if(k == mykey+input_size){
                PLA_[index_PLAi][index_PLAj] = data[i][k];
				//printf("%c",data[i][k]);
                index_PLAj++;
            }
        }
		//printf("\n");
        PLA_[index_PLAi][index_PLAj] = '\n';
        index_PLAj++;
        index_PLAi++;
    }

	// for(int i=0;i<row_count;i++){
	// 	for(int j=0;j<strlen(PLA_[0]);j++){
	// 		printf("%c",PLA_[i][j]);
	// 	}
	// }

    //write file
    char newPLA1name[255];

    strcpy(newPLA1name,argv[opt]);
    strcat(newPLA1name,newname);
    char PLAbuffer[255] = ".o ";
    strcat(PLAbuffer,"1\n");

    FILE *newPLA = fopen(newPLA1name,"w");

    fputs(input_buf_k,newPLA);
    fputs(PLAbuffer,newPLA);
	
    for(int i=0;i<row_count;i++){
        for(int j=0;j<strlen(PLA_[0]);j++){
            fputc(PLA_[i][j],newPLA);
        }
    }
    fclose(newPLA);
	memset(PLA_, 0, sizeof(PLA_[0][0]) * row_count * col_count);

	fname = newPLA1name;
	FILE *newfp = fopen(newPLA1name,"r");
	//------------------my code--------------------
    ...
    if (read_pla(newfp, needs_dcset, needs_offset, input_type, PLA) == EOF) {
