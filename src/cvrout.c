...
void fprint_pla(FILE *fp, pPLA PLA, int output_type , char newfilename[])
{
	FILE *newfilefp=fopen(newfilename,"w");
	...
	fpr_header(fp, PLA, output_type ,newfilefp);
...

void fpr_header(FILE *fp, pPLA PLA, int output_type ,FILE *newfilefp)
{
    register int i, var;
    int first, last;
	...
	fputs(".i ",newfilefp);
	sprintf(mybuf,"%d",cube.num_binary_vars);
	strcat(mybuf,"\n");
	fputs(mybuf,newfilefp);
...
void print_cube(register FILE *fp, register pset c, register char *out_map ,FILE *newfilefp)
{
